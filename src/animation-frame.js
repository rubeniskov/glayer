...
var vendors = ['moz', 'webkit'],
    ...
    mozRAF = global.mozRequestAnimationFrame,
    mozCAF = global.mozCancelAnimationFrame,
...
function hasIOS6RequestAnimationFrameBug() {
    if (!global.navigator)
        return false;
    var webkitRAF = global.webkitRequestAnimationFrame,
        ...
        hasNoNavigationTiming = global.performance ? false : true,
...
function queryRequestAnimationFrame() {
    if (Array.prototype.filter) {
        assignedRequestAnimationFrame = global['request' + aF] ||
            global[vendors.filter(function(vendor) {
                if (global[vendor + rqAF] !== undefined)
                    return vendor;
...
function queryCancelAnimationFrame() {
    ...
    function prefixedCancelAnimationFrame(prefixedNames, i) {
        ...
        for (; i < prefixedNames.length; i++) {
            if (global[prefixedNames[i]]) {
                cancellationFunction = global[prefixedNames[i]];
                break;
            }
        }
    ...
    assignedCancelAnimationFrame = global['cancel' + aF] ||
...
export default {
    requestAnimationFrame: getRequestFn(),
    cancelAnimationFrame: getCancelFn(),
    polyfill: function(){
        global.requestAnimationFrame = getRequestFn();
        global.cancelAnimationFrame = getCancelFn();
    }
};
